 ...
 * WLAN Device
 */
static const char athId[] __attribute__ ((unused)) = "$Id: //depot/sw/releases/olca2.2/host/os/linux/ar6000_drv.c#52 $";
...
char *fm_path = NULL;
char *tgt_fw = "ath6k/AR6002/athwlan.bin.z77";
char *tgt_patch = "ath6k/AR6002/data.patch.hw2_0.bin";
char *tcmd_fw = "ath6k/AR6002/athtcmd_ram.bin";
char *art_fw = "ath6k/AR6002/device.bin";
char *eeprom_bin = "ath6k/AR6002/eeprom.bin";
char *eeprom_data = "ath6k/AR6002/eeprom.data";


#ifdef REGION_CODE_FILE_USED
char *reg_file = "ath6k/AR6002/reg_code";
#else
char *reg_file = NULL;
#endif
...
char *eeprom_file = "ath6k/AR6002/calData_ar6102_15dBm.bin";
/*char *eeprom_file = "ath6k/AR6002/fakeBoardData_AR6002.bin"; */
#else
...
static void firmware_transfer(AR_SOFTC_T *ar,HIF_DEVICE *device, char* filename, A_UINT32 address, A_BOOL isCompressed)
{
    struct file     *filp;
...
Transfer_DONE:
    ...
    set_fs(oldfs);
#else
    printk("XXXXXXXXX: ar->osDevInfo.pOSDevice=%x\n",ar->osDevInfo.pOSDevice);
   ...
   if ((request_firmware(&fw_entry, filename, (struct device *)ar->osDevInfo.pOSDevice)) != 0)
    ...
    if (isCompressed) {
        ret = BMIFastDownload(device, address, (A_UCHAR *)fw_entry->data, fw_entry->size);
    } else {
        ret = BMIWriteMemory(device, address, (A_UCHAR *)fw_entry->data, fw_entry->size);
    }
                      ...
                      char *buf, loff_t pos, size_t count)
{
    ...
    AR_DEBUG_PRINTF("BMI: Read %d bytes\n", count);
    for (index=0; index < MAX_AR6000; index++) {
        ar = (AR_SOFTC_T *)netdev_priv(ar6000_devices[index]);
                       ...
                       char *buf, loff_t pos, size_t count)
{
    ...
    AR_DEBUG_PRINTF("BMI: Write %d bytes\n", count);
    for (index=0; index < MAX_AR6000; index++) {
        ar = (AR_SOFTC_T *)netdev_priv(ar6000_devices[index]);
        osDevInfo = &ar->osDevInfo;
        if (kobj == (&(((struct device *)osDevInfo->pOSDevice)->kobj))) {
...
ar6000_sysfs_bmi_init(AR_SOFTC_T *ar)
{
    ...
    AR_DEBUG_PRINTF("BMI: Creating sysfs entry\n");
    ...
    if (A_FAILED(status)) {
        AR_DEBUG_PRINTF("BMI: Failed to get OS device info from HIF\n");
        return A_ERROR;
    }
    ...
    {
        AR_DEBUG_PRINTF("BMI: Failed to create entry for bmi in sysfs filesystem\n");
        return A_ERROR;
    }
...
ar6000_sysfs_bmi_deinit(AR_SOFTC_T *ar)
{
    AR_DEBUG_PRINTF("BMI: Deleting sysfs entry\n");
...
#define bmifn(fn) do { \
    if ((fn) < A_OK) { \
        AR_DEBUG_PRINTF("BMI operation failed: %d\n", __LINE__); \
        return A_ERROR; \
    } \
} while(0)
...
/* ATHENV */
    ...
    if (tgt_fw != NULL) {
        ...
        firmware_transfer(ar, ar->arHifDevice, eeprom_data, 0x502070, FALSE);
        firmware_transfer(ar, ar->arHifDevice, eeprom_bin, 0x5140f0, FALSE);
        value = 1;
        ...
        if ((tgt_fw[strlen(tgt_fw) - 3] == 'z')
         ...
         && (tgt_fw[strlen(tgt_fw) - 1] == '7')) {
            firmware_transfer(ar, ar->arHifDevice, tgt_fw, 0x502070, TRUE);
    } else {
            firmware_transfer(ar, ar->arHifDevice, tgt_fw, 0x502070, FALSE);
        }

        /* WLAN patch DataSets */
        firmware_transfer(ar , ar->arHifDevice, tgt_patch, 0x52d6c8, FALSE);
...
ar6000_avail_ev(void *context, void *hif_handle)
{
	...
	A_MEMZERO(&ar->osDevInfo, sizeof(HIF_DEVICE_OS_DEVICE_INFO));
        if ( A_SUCCESS( HIFConfigureDevice(hif_handle, HIF_DEVICE_GET_OS_DEVICE,
                        &ar->osDevInfo, sizeof(osDevInfo))) ) {
            SET_NETDEV_DEV(dev, ar->osDevInfo.pOSDevice);
        }
    }
