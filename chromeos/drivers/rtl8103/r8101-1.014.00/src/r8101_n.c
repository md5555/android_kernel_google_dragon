...
/*
 * Only detect rtl8103 in such a way so as not to collide
 * with the upstream driver r8169.
 */
#define RTL_8103_ONLY 1
...
static const struct {
	...
	_R("RTL8401",  CFG_METHOD_8, 0xff7e1880),
};
#undef _R
...
static inline void rtl8101_delete_link_down_check_timer(struct net_device *dev, struct timer_list *timer);
static inline void rtl8101_request_link_down_check_timer(struct net_device *dev);
static void rtl8101_link_down_check_timer(unsigned long __opaque);
...
static void rtl8101_aspm_timer(unsigned long __opaque);
static void rtl8101_aspm_fix1(struct net_device *dev);
static void rtl8101_aspm_fix2(struct net_device *dev);

static int rtl8101_open(struct net_device *dev);
static int rtl8101_fnf2_recover(struct net_device *dev);
...
void rtl8101_rar_set(struct rtl8101_private *tp, uint8_t *addr, uint32_t index);

static void rtl8101_nic_reset(struct net_device *dev);
...
static void
rtl8101_ephy_write(void __iomem *ioaddr, 
		   int RegAddr, 
		   int value)
{
	int i;
...
static int 
rtl8101_ephy_read(void __iomem *ioaddr, 
		  int RegAddr)
{
	int i;
	u16 value = 0xffff;
...
static void
rtl8101_csi_write(void __iomem *ioaddr, 
		   int addr,
		   int value)
{
	int i;
...
static int 
rtl8101_csi_read(void __iomem *ioaddr, 
		 int addr)
{
	int i, value = -1;
...
static void 
rtl8101_asic_down(struct net_device *dev)
{
			  ...
			  void __iomem *ioaddr)
{
	...
	/* Don't let driver set carrier on when network is disabled by fnf2 */ 
	...
	if (tp->link_ok(dev)) {
		...
		if (tp->mcfg == CFG_METHOD_8) {
			mdio_write(ioaddr, 0x1F, 0x0000);
			mdio_write(ioaddr, 0x11, mdio_read(ioaddr, 0x11) | BIT_12);
			mdio_write(ioaddr, 0x1F, 0x0002);
			mdio_write(ioaddr, 0x0F, mdio_read(ioaddr, 0x0F) | BIT_0 | BIT_1);
			mdio_write(ioaddr, 0x1F, 0x0000);
		}
	}

	rtl8101_aspm_fix2(dev);
		    ...
		    struct sk_buff *skb)
{
	...
	if (tp->vlgrp && (vlan_sig == ETH_P_8021Q)) {
		...
		rtl8101_rx_hwaccel_skb(skb, tp->vlgrp, swab16(rx_vlan));
		ret = 0;
	} else {
		ret = -1;
	}
...
static int 
rtl8101_get_stats_count(struct net_device *dev)
{
...
rtl8101_hw_phy_config(struct net_device *dev)
{
	...
	if (tp->mcfg == CFG_METHOD_4) {
		...
		mdio_write(ioaddr, 0x10, mdio_read(ioaddr, 0x10) | 0x8000);
...
{
	struct rtl8101_private *tp = netdev_priv(dev);
	struct timer_list *timer = &tp->link_down_check_timer;
	...
	init_timer(timer);
	timer->expires = jiffies + RTL8101_LINK_DOWN_TIMEOUT;
	timer->data = (unsigned long)(dev);
	timer->function = rtl8101_link_down_check_timer;
	add_timer(timer);
}

static inline void rtl8101_delete_aspm_timer(struct net_device *dev, struct timer_list *timer)
{
	struct rtl8101_private *tp = netdev_priv(dev);
...
static inline void rtl8101_request_aspm_timer(struct net_device *dev)
{
	struct rtl8101_private *tp = netdev_priv(dev);
	struct timer_list *timer = &tp->aspm_timer;

	if (tp->mcfg != CFG_METHOD_4) 
		return;

	init_timer(timer);
	timer->expires = jiffies + RTL8101_ASPM_TIMEOUT;
	timer->data = (unsigned long)(dev);
	timer->function = rtl8101_aspm_timer;
	add_timer(timer);
}
			...
			void *p)
{
	...
	rtl8101_rar_set(tp, tp->mac_addr, 0);
...
#else
			dev_printk(KERN_DEBUG, &pdev->dev, "unknown chip version, unloading\n");
			rc = -ENOMEM;
			goto err_out_free_res;
#endif
		}
...
static void
rtl8101_link_down_check_timer(unsigned long __opaque)
{
	...
	data = rtl8101_csi_read(ioaddr, 0x70c);
	if (tp->link_ok(dev)) {
		if (data != 0x27000000) {
			rtl8101_csi_write(ioaddr, 0x70c, 0x27000000);
		}
	} else {
		if (data != 0x17000000) {
			rtl8101_csi_write(ioaddr, 0x70c, 0x17000000);
		}
	}

	mod_timer(timer, jiffies + RTL8101_LINK_DOWN_TIMEOUT);
}

static void
rtl8101_aspm_timer(unsigned long __opaque)
{
	struct net_device *dev = (struct net_device *)__opaque;
	struct rtl8101_private *tp = netdev_priv(dev);
	struct timer_list *timer = &tp->aspm_timer;
	unsigned long timeout = RTL8101_ASPM_TIMEOUT;
...
rtl8101_aspm_fix1(struct net_device *dev)
{
	...
	if (tp->mcfg == CFG_METHOD_4) {
		data = rtl8101_csi_read(ioaddr, 0x110);
...
static void
rtl8101_aspm_fix2(struct net_device *dev)
{
	struct rtl8101_private *tp = netdev_priv(dev);
	void __iomem *ioaddr = tp->mmio_addr;
	int data;
	...
	csi_tmp = rtl8101_csi_read(ioaddr, 0x70c) & 0x00ffffff;
	if (tp->link_ok(dev)) {
		rtl8101_csi_write(ioaddr, 0x70c, csi_tmp | 0x17000000);
	} else {
		rtl8101_csi_write(ioaddr, 0x70c, csi_tmp | 0x3F000000);
	}
}
...
static int rtl8101_fnf2_recover(struct net_device *dev)
{
	...
	tp->RxDescArray = pci_alloc_consistent(pdev, R8101_RX_RING_BYTES,
					       &tp->RxPhyAddr);
	if (!tp->RxDescArray)
		goto err_free_tx_realtek;

	retval = rtl8101_init_ring(dev);
	if (retval < 0)
		goto err_free_rx_realtek;
...
static int rtl8101_open(struct net_device *dev)
{
	...
	retval = request_irq(dev->irq, rtl8101_interrupt, SA_SHIRQ, dev->name, dev);
	...
	tp->counters = pci_alloc_consistent(pdev, sizeof(*tp->counters), &tp->paddr);
	if (!tp->counters)
		goto err_free_rx;

	RTL_W32(CounterAddrHigh, (u64)tp->paddr >> 32);
	RTL_W32(CounterAddrLow, (u64)tp->paddr & (DMA_32BIT_MASK | CounterDump | BIT_0));
	...
	pci_read_config_byte(pdev, 0x80, &data);
	if (data & ((1 << 1) | (1 << 0))) {
		rtl8101_request_link_down_check_timer(dev);
		tp->link_down_check_timer_enable = 1;
	}

	if (tp->esd_flag == 0) {
		rtl8101_request_aspm_timer(dev);
...
err_free_counters:
	pci_free_consistent(pdev, sizeof(*tp->counters), tp->counters, tp->paddr);
	...
	case DSM_MAC_INIT:
		if ((tp->mcfg == CFG_METHOD_4) ||
			...
			if (RTL_R8(MACDBG) & 0x80) {
				mdio_write(ioaddr, 0x1f, 0x0000);
				mdio_write(ioaddr, 0x11, mdio_read(ioaddr, 0x11) & ~(1 << 12));
...
rtl8101_hw_start(struct net_device *dev)
{
	...
	void __iomem *ioaddr = tp->mmio_addr;
	struct pci_dev *pdev = tp->pci_dev;
	u8 device_control, link_control, options1, options2;
	u32 csi_tmp;
...
static inline u32 
rtl8101_tso(struct sk_buff *skb, 
	    struct net_device *dev)
{
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,0)
		   ...
		   struct net_device *dev)
{
	...
	opts1 = DescOwn | rtl8101_tso(skb, dev);
...
static void 
rtl8101_pcierr_interrupt(struct net_device *dev)
{
	struct rtl8101_private *tp = netdev_priv(dev);
	struct pci_dev *pdev = tp->pci_dev;
	void __iomem *ioaddr = tp->mmio_addr;
...
#endif
{
	...
	u16 intr_clean_mask = SYSErr | PCSTimeout | SWInt | 
			      LinkChg | RxDescUnavail | TxErr | TxOK | 
			      RxErr | RxOK;
...
static int rtl8101_poll(napi_ptr napi, napi_budget budget)
{
	...
	if (work_done < work_to_do) {
		...
		tp->intr_mask = rtl8101_intr_mask;
		/*
...
rtl8101_down(struct net_device *dev)
{
	...
	rtl8101_delete_aspm_timer(dev, &tp->aspm_timer);
	rtl8101_delete_esd_timer(dev, &tp->esd_timer);

	pci_read_config_byte(pdev, 0x80, &data);
	if ((data & ((1 << 1) | (1 << 0))) && tp->link_down_check_timer_enable) {
		rtl8101_delete_link_down_check_timer(dev, &tp->link_down_check_timer);
	}
...
//	RTL_W32(RxMissed, 0);
	 ...
	 */
	if (RTL_R16(IntrMask)) {
		if ((u16)rtl8101_csi_read(ioaddr, 0x10) != 0xffff) {
			goto core_down;
		}
	}
...
static int 
rtl8101_close(struct net_device *dev)
{
	struct rtl8101_private *tp = netdev_priv(dev);
	struct pci_dev *pdev = tp->pci_dev;
	void __iomem *ioaddr = tp->mmio_addr;
