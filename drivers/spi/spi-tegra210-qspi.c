...
static int check_and_clear_fifo(struct tegra_qspi_data *tqspi)
{
	...
	if ((status & QSPI_FIFO_EMPTY) != QSPI_FIFO_EMPTY) {
		...
		timeout = jiffies + usecs_to_jiffies(QSPI_FIFO_FLUSH_MAX_DELAY);
...
static int tegra_qspi_get_bus_width(struct spi_transfer *t)
{
	if (t->tx_buf) {
		switch (t->tx_nbits) {
		case SPI_NBITS_QUAD:
		...
		bool is_single_xfer)
{
	...
	if (cdata) {
		if ((t->len - tqspi->cur_pos) >  cdata->x1_len_limit) {
			num_dummy_cycles =  cdata->x4_dummy_cycle;
			speed = cdata->x4_bus_speed;
		} else {
			num_dummy_cycles =  cdata->x1_dummy_cycle;
	...
	ret = tegra_qspi_validate_request(spi, tqspi, t);
	if (ret)
		return ret;
	...
	command1 |= QSPI_INTERFACE_WIDTH(tegra_qspi_get_bus_width(t));
...
static void tegra_qspi_set_cs(struct spi_device *spi,
		bool enable)
{
	struct tegra_qspi_data *tqspi = spi_master_get_devdata(spi->master);
	int ret;
	...
	*tegra_qspi_get_cdata_dt(struct spi_device *spi)
{
	...
	prop = of_get_property(data_np, "nvidia,x1-dummy-cycle", NULL);
	if (prop)
		cdata->x1_dummy_cycle = be32_to_cpup(prop);

	prop = of_get_property(data_np, "nvidia,x4-bus-speed", NULL);
	if (prop)
		cdata->x4_bus_speed = be32_to_cpup(prop);

	prop = of_get_property(data_np, "nvidia,x4-dummy-cycle", NULL);
	if (prop)
		cdata->x4_dummy_cycle = be32_to_cpup(prop);
...
static int tegra_qspi_probe(struct platform_device *pdev)
{
	...
	master->set_cs  = tegra_qspi_set_cs;
	...
	tqspi->irq = qspi_irq;
	ret = devm_request_threaded_irq(&pdev->dev, tqspi->irq, tegra_qspi_isr,
	...
	tqspi->rst = devm_reset_control_get(&pdev->dev, "qspi");
	if (IS_ERR(tqspi->rst)) {
