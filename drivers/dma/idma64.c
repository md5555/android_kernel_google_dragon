...
static int idma64_device_control(struct dma_chan *chan, enum dma_ctrl_cmd cmd, 
			  unsigned long arg)
{
	struct idma64_chan *idma64c = to_idma64_chan(chan);
	unsigned long flags;
	...
	if (cmd == DMA_SLAVE_CONFIG) {
		...
		idma64_slave_config(chan, config);
		return 0;
	}
	...
	/* The remainder of the function terminates the transfer. */
	spin_lock_irqsave(&idma64c->vchan.lock, flags);
	idma64_chan_deactivate(idma64c);
	idma64_stop_transfer(idma64c);
	if (idma64c->desc) {
...
static int idma_device_slave_caps(struct dma_chan *dchan,
...
static int idma64_probe(struct idma64_chip *chip)
{
	...
	idma64->dma.device_control = idma64_device_control;

	idma64->dma.device_slave_caps = idma_device_slave_caps;
