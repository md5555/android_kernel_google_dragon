 ...
 *
 * Usage:	Application Programming Interface for SMB380 configuration
   and data read out
 *
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 you may not use this file except in compliance with the License and
 the following stipulations. The Apache License , Version 2.0 is
 applicable unless otherwise stated by the stipulations of
 ...
 *
*Common:
*This Work is developed for the consumer goods industry.
*It may only be used within the parameters of the respective valid
*product data sheet.  The Work provided with the express
*understanding that there is no warranty of fitness for a
*particular purpose. It is not fit for use in life-sustaining,
*safety or security sensitive systems or any system or device
*that may lead to bodily harm or property damage if the system
*or device malfunctions. In addition, the Work is not fit for use in
*products which interact with motor vehicle systems.
*The resale and/or use of the Work are at the purchaser
*own risk and his own responsibility. The examination of fitness for
*the intended use is the sole responsibility of the Purchaser.

*The purchaser shall indemnify Bosch Sensortec from all third party
*claims, including any claims for incidental, or consequential
*damages, arising from any Work or Derivative Work use not covered
*by the parameters of the respective valid product data sheet or not
*approved by Bosch Sensortec and reimburse Bosch Sensortec for all
*costs in connection with such claims.

*The purchaser must monitor the market for the purchased Work and
*Derivative Works, particularly with regard to product safety and
...
*may vary from the valid technical specifications of the product
*series. They are therefore not intended or fit for resale to third
*parties or for use in end products. Their sole purpose is internal
*client testing. The testing of an engineering sample may in no way
*replace the testing of a product series. Bosch Sensortec
*assumes no liability for the use of engineering samples.
*By accepting the engineering samples, the Purchaser agrees to
*indemnify Bosch Sensortec from all claims arising from the use of
*engineering
*samples.

*Special:
*This Work and any related information
*(hereinafter called "Information") is provided free of charge
*for the sole purpose to support your application work.
*The Woek and Information is subject to the following terms and
*conditions:

*The Work is specifically designed for the exclusive use for
*Bosch Sensortec products by personnel who have special experience
*and training. Do not use this Work or Derivative Works if you do
*not have the proper experience or training. Do not use this Work or
*Derivative Works fot other products than Bosch Sensortec products.

*The Information provided is believed to be accurate and reliable.
*Bosch Sensortec assumes no responsibility for the consequences of
*use of such Information nor for any infringement of patents or
*other rights of third parties which may result from its use.
*No license is granted by implication or otherwise under any patent
*or patent rights of Bosch. Specifications mentioned in the
...
/** Define the calling convention of YOUR bus communication routine.
	...
	write_my_bus_xy(unsigned char device_addr, unsigned char
	register_addr, unsigned char * data, unsigned char length);

	The SMB380_WR_FUNC_PTR would equal:

		#define	SMB380_WR_FUNC_PTR char (* bus_write)
		(unsigned char, unsigned char, unsigned char *, unsigned char)

	Parameters can be mixed as needed refer to the
	\ref SMB380_BUS_WRITE_FUNC  macro.


*/
#define SMB380_WR_FUNC_PTR \
char (*bus_write)(unsigned char, unsigned char *, unsigned char)
...
/** link makro between API function calls and bus write function
	...
	If the parameters are differently ordered or
	your communication function like I2C need to know the device address,
    ...
    This macro lets all API functions call YOUR communication routine
    in a way that equals your definition in the
		\ref SMB380_WR_FUNC_PTR definition.
...
/** Define the calling convention of YOUR bus communication routine.
    ...
    #define	SMB380_RD_FUNC_PTR char (* bus_read)(unsigned char,
    unsigned char, unsigned char *, unsigned char)

	Parameters can be mixed as needed refer to the
	\ref SMB380_BUS_READ_FUNC  macro.
...
#define SMB380_RD_FUNC_PTR \
char (*bus_read)(unsigned char, unsigned char *, unsigned char)


/** link makro between API function calls and bus read function
	\note The bus write function can change since this
	is a system dependant issue.
    ...
    This macro lets all API functions call YOUR communication
     routine in a way that equals your definition in the
     \ref SMB380_WR_FUNC_PTR definition.

	\note: this macro also includes the "MSB='1'"
	for reading SMB380 addresses.
...
/** SMB380 acceleration data
	\brief Structure containing acceleration values for x,y
	and z-axis in signed short

*/

struct smb380_acc_t{
	short x, /**x-axis acce data sign extended. Range -512 to 511. */
	y, /**y-axis acce data sign extended. Range -512 to 511. */
	z; /**z-axis acce data sign extended. Range -512 to 511. */
};


/** SMB380 image registers data structure
	\brief Register type that contains all SMB380 image
	registers from address 0x0b to 0x15
	This structure can hold the complete image data of SMB380

*/
struct  smb380_regs_t{
unsigned char
/**<  image address 0x0b: interrupt enable bits, low-g settings */
smb380_conf1 ,
/**<  image address 0x0c: low-g threshold, depends on selected g-range */
lg_threshold,
/**<  image address 0x0d: low-g duration in ms */
lg_duration,
/**<  image address 0x0e: high-g threshold, depends on selected g-range */
hg_threshold,
/**<  image address 0x0f: high-g duration in ms */
hg_duration,
/**<  image address 0x10: any motion threshold */
motion_thrs,
/**<  image address 0x11: low-g and high-g hysteresis register */
hysteresis,
/**<  image address 0x12: customer reserved register 1 */
customer1,
/**<  image address 0x13: customer reserved register 2  */
customer2,
/**<  image address 0x14: range and bandwidth selection register */
range_bwidth,
/**<  image address 0x15: spi4, latched interrupt, auto-wake-up configuration */
smb380_conf2,
/**<  image address 0x16: offset_x LSB and x-axis gain settings */
offs_gain_x,
/**<  image address 0x17: offset_y LSB and y-axis gain settings */
offs_gain_y,
/**<  image address 0x18: offset_z LSB and z-axis gain settings */
offs_gain_z,
/**<  image address 0x19: offset_t LSB and temperature gain settings */
offs_gain_t,
/**<  image address 0x1a: offset_x calibration MSB register */
offset_x,
/**<  image address 0x1b: offset_y calibration MSB register */
offset_y,
/**<  image address 0x1c: offset_z calibration MSB register */
offset_z,
/**<  image address 0x1d: temperature calibration MSB register */
offset_t;
};


/** smb380 typedef structure
	\brief This structure holds all relevant
	information about SMB380 and links communication to the
*/

struct smb380_t{
	struct smb380_regs_t *image;
	unsigned char mode;
...
/** Macro to convert floating point low-g-thresholds
	in G to 8-bit register values.<br>
  * Example: SMB380_LG_THRES_IN_G( 0.3, 2.0) generates
...
/** Macro to convert floating point high-g-thresholds in G
	to 8-bit register values.<br>
  * Example: SMB380_HG_THRES_IN_G( 1.4, 2.0) generates
	the register value for 1.4G threshold in 2G mode.
  * \brief convert g-values to 8-bit value
 */
#define SMB380_HG_THRES_IN_G(gthres, range)  ((256 * gthres) / range)

/** Macro to convert floating point low-g-hysteresis in
	G to 8-bit register values.<br>
  * Example: SMB380_LG_HYST_THRES_IN_G( 0.2, 2.0) generates
	the register value for 0.2G threshold in 2G mode.
  * \brief convert g-values to 8-bit value
 */
#define SMB380_LG_HYST_IN_G(ghyst, range) ((32 * ghyst) / range)

/** Macro to convert floating point high-g-hysteresis in
	G to 8-bit register values.<br>
  * Example: SMB380_HG_HYST_THRES_IN_G( 0.2, 2.0)
...
/** Macro to convert floating point G-thresholds to
	8-bit register values<br>
  * Example: SMB380_ANY_MOTION_THRES_IN_G( 1.2, 2.0)
...
#define SMB380_CONF1_INT_MSK	\
((1<<SMB380_ALERT__POS) | (1<<SMB380_EN_ANY_MOTION__POS) | \
(1<<SMB380_ENABLE_HG__POS) | (1<<SMB380_ENABLE_LG__POS))

#define SMB380_CONF2_INT_MSK	\
((1<<SMB380_ENABLE_ADV_INT__POS) | (1<<SMB380_NEW_DATA_INT__POS) | \
(1<<SMB380_LATCH_INT__POS))

/* Function prototypes */

int smb380_init(struct smb380_t *);
int smb380_set_image(struct smb380_regs_t *);
int smb380_get_image(struct smb380_regs_t *);
...
int smb380_read_accel_xyz(struct smb380_acc_t *);
