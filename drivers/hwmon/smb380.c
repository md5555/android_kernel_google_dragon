...
*
* Usage:	Application Programming Interface for
*         SMB380 configuration and data read out
*
*
* Licensed under the Apache License, Version 2.0 (the "License");
  you may not use this file except in compliance with the License
  and the following stipulations.
  The Apache License , Version 2.0 is applicable unless
...
*The Work provided with the express understanding that there is no
*warranty of fitness for a particular purpose.
*It is not fit for use in life-sustaining, safety or security
*sensitive systems or any system or device that may lead to bodily
*harm or property damage if the system or device malfunctions.
*In addition, the Work is not fit for use in products which interact
*with motor vehicle systems. The resale and/or use of the Work are at
*the purchaser own risk and his own responsibility. The examination
*of fitness for the intended use is the sole responsibility of the
*Purchaser.
*
*The purchaser shall indemnify Bosch Sensortec from all third party
*claims, including any claims for incidental, or consequential damages,
*arising from any Work or Derivative Work use not covered by the
*parameters of the respective valid product data sheet or not approved
*by Bosch Sensortec and reimburse Bosch Sensortec for all costs in
...
*may vary from the valid technical specifications of the product series.
*They are therefore not intended or fit for resale to third
...
*is provided free of charge for the sole purpose to support your
*application work. The Woek and Information is subject to the following
*terms and conditions:
*
*The Work is specifically designed for the exclusive use for Bosch
*Sensortec products by personnel who have special experience and
*training. Do not use this Work or Derivative Works if you do not
*have the proper experience or training. Do not use this Work or
...
static struct smb380_t *p_smb380;  /**< pointer to SMB380 device structure  */
...
int smb380_init(struct smb380_t *smb380)
{
	int comres = 0;
	unsigned char data;

	p_smb380 = smb380;                      /* assign smb380 ptr */
	comres = p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr,
						 SMB380_CHIP_ID__REG,
						 &data,
						 1);
...
int smb380_soft_reset()
{
	...
	comres = p_smb380->SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr,
						 SMB380_SOFT_RESET__REG,
						 &data,
						 1);
	return comres;
}
...
/** copy image from image structure to SMB380 image memory
   \param smb380_image Pointer to smb380_regs_t
   \return result of bus communication function
*/
int smb380_set_image(struct smb380_regs_t *smb380_image)
{
	int comres;
	unsigned char data;
	if (p_smb380 == 0)
...
/** read out image from SMB380 and store it to smb380_regs_t structure
   \param smb380_image pointer to smb380_regs_t
   \return result of bus communication function

*/
int smb380_get_image(struct smb380_regs_t *smb380_image)
{
...
/** read out offset data from
   ...
   \param *offset pointer to offset value (offset is
	in offset binary representation
   \return result of bus communication function
...
/** write offset data to SMB380 image
   \param xyz select axis x=0, y=1, z=2
   \param offset value to write to eeprom(offset is in
	offset binary representation
   \return result of bus communication function
   \note use smb380_set_ee_w() function to enable access to
	offset registers in EEPROM space
*/
...
/** write byte to SMB380 EEPROM
   \param addr address to write to
    (image addresses are automatically extended to EEPROM space)
...
int smb380_write_ee(unsigned char addr, unsigned char data)
{
	...
	comres = smb380_set_ee_w(SMB380_EE_W_ON);
	 /* add eeprom address offset to image address if not applied */
	addr |= 0x20;
...
/** set SMB380 internal filter bandwidth
   ...
   \return result of bus communication function
   \see #define SMB380_BW_25HZ, SMB380_BW_50HZ, SMB380_BW_100HZ,
	SMB380_BW_190HZ, SMB380_BW_375HZ, SMB380_BW_750HZ, SMB380_BW_1500HZ
...
/** set SMB380 auto wake up pause
  \param wup wake_up_pause parameters
	\return result of bus communication function
	\see SMB380_WAKE_UP_PAUSE_20MS, SMB380_WAKE_UP_PAUSE_80MS,
	     SMB380_WAKE_UP_PAUSE_320MS, SMB380_WAKE_UP_PAUSE_2560MS
...
/** set low-g interrupt threshold
   ...
   \ref SMB380_LG_THRES_IN_G() for range to register value conversion
			is available.
...
/** configure low-g duration value
	...
	\see smb380_get_low_g_duration(), smb380_get_high_g_duration(),
	     smb380_set_high_g_duration()
...
/** read out low-g duration value from sensor image
	\param dur low-g duration in miliseconds
	\see smb380_set_low_g_duration(), smb380_get_high_g_duration(),
	    smb380_set_high_g_duration()
...
/** set low-g interrupt threshold
   \param th set the threshold
   \note the threshold depends on configured range. A macro
   \ref SMB380_HG_THRES_IN_G() for range to register value conversion
		is available.
...
/** configure high-g duration value
	...
	\see  smb380_get_high_g_duration(), smb380_set_low_g_duration(),
	      smb380_get_low_g_duration()
...
/** read out high-g duration value from sensor image
	\param dur high-g duration in miliseconds
	\see  smb380_set_high_g_duration(), smb380_get_low_g_duration(),
	      smb380_set_low_g_duration(),
...
/**  set threshold value for any_motion feature
		\param th set the threshold a macro
		\ref SMB380_ANY_MOTION_THRES_IN_G()  is available for that
...
/**  set counter value for any_motion feature
		...
		\see SMB380_ANY_MOTION_DUR_1, SMB380_ANY_MOTION_DUR_3,
		     SMB380_ANY_MOTION_DUR_5, SMB380_ANY_MOTION_DUR_7
*/
...
/** set the interrupt mask for SMB380's interrupt features in one mask
	...
	\see SMB380_INT_ALERT, SMB380_INT_ANY_MOTION, SMB380_INT_EN_ADV_INT,
	     SMB380_INT_NEW_DATA, SMB380_INT_LATCH, SMB380_INT_HG, SMB380_INT_LG
*/
...
int smb380_read_accel_x(short *a_x)
{
	...
	comres = p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr,
						SMB380_ACC_X_LSB__REG,
						data,
						2);
	*a_x = (SMB380_GET_BITSLICE(data[0], SMB380_ACC_X_LSB)) |
	       (SMB380_GET_BITSLICE(data[1], SMB380_ACC_X_MSB)<<
		SMB380_ACC_X_LSB__LEN);

	*a_x = *a_x << (sizeof(short)*8-(SMB380_ACC_X_LSB__LEN+
					 SMB380_ACC_X_MSB__LEN));
...
int smb380_read_accel_y(short *a_y)
{
	...
	comres = p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr,
						SMB380_ACC_Y_LSB__REG,
						data,
						2);

	*a_y = (SMB380_GET_BITSLICE(data[0], SMB380_ACC_Y_LSB)) |
	       (SMB380_GET_BITSLICE(data[1], SMB380_ACC_Y_MSB)<<
		SMB380_ACC_Y_LSB__LEN);

	*a_y = *a_y << (sizeof(short)*8-(SMB380_ACC_Y_LSB__LEN+
					 SMB380_ACC_Y_MSB__LEN));
...
int smb380_read_accel_z(short *a_z)
{
	...
	comres = p_smb380->SMB380_BUS_READ_FUNC(p_smb380->dev_addr,
						SMB380_ACC_Z_LSB__REG,
						data,
						2);

	*a_z = (SMB380_GET_BITSLICE(data[0], SMB380_ACC_Z_LSB)) |
	       (SMB380_GET_BITSLICE(data[1], SMB380_ACC_Z_MSB)<<
		SMB380_ACC_Z_LSB__LEN);

	*a_z = *a_z << (sizeof(short)*8-(SMB380_ACC_Z_LSB__LEN+
					 SMB380_ACC_Z_MSB__LEN));
...
/** 8 bit temperature data readout
	...
	\note: an output of 0 equals -30, 1 LSB equals 0.5
*/
...
/** X,Y and Z-axis acceleration data readout
	\param *acc pointer to \ref smb380_acc_t structure for x,y,z data readout
	\note data will be read by multi-byte protocol into a 6 byte structure
*/
int smb380_read_accel_xyz(struct smb380_acc_t *acc)
{
	int comres;
	...
	acc->x = acc->x << (sizeof(short)*8-(SMB380_ACC_X_LSB__LEN+
					     SMB380_ACC_X_MSB__LEN));
	...
	acc->y = acc->y << (sizeof(short)*8-(SMB380_ACC_Y_LSB__LEN +
					     SMB380_ACC_Y_MSB__LEN));
	...
	acc->z = acc->z << (sizeof(short)*8-(SMB380_ACC_Z_LSB__LEN+
					     SMB380_ACC_Z_MSB__LEN));
...
/** check current interrupt status from interrupt status
	register in SMB380 image register
	\param *ist pointer to interrupt status byte
	\see SMB380_INT_STATUS_HG, SMB380_INT_STATUS_LG,
	     SMB380_INT_STATUS_HG_LATCHED, SMB380_INT_STATUS_LG_LATCHED,
	     SMB380_INT_STATUS_ALERT, SMB380_INT_STATUS_ST_RESULT
*/
...
/** enable/ disable any_motion interrupt feature
		\param onoff enable=1, disable=0
		\note for any_motion interrupt feature usage see also
		\ref smb380_set_advanced_int()
*/
