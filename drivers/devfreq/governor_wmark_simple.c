...
struct wmark_gov_info {
	...
	enum devfreq_watermark_event_type event;
...
{
	if (wmarkinfo->freq_index < (wmarkinfo->freq_count - 1))
		wmarkinfo->freq_index++;

	return wmarkinfo->freqlist[wmarkinfo->freq_index];
}

static unsigned long freqlist_down(struct wmark_gov_info *wmarkinfo,
				   unsigned long curr_freq)
{
	if (wmarkinfo->freq_index > 0)
		wmarkinfo->freq_index--;
					 ...
					 unsigned long *freq)
{
	...
	case DEVFREQ_WATERMARK_EVENT_ABOVE:
		*freq = freqlist_up(wmarkinfo, dev_stat.current_frequency);
		...
		break;
	case DEVFREQ_WATERMARK_EVENT_BELOW:
	...
	wmarkinfo->event = DEVFREQ_WATERMARK_EVENT_NONE;
...
{
	struct wmark_gov_info *wmarkinfo = df->data;
	enum devfreq_watermark_event_type *type = wmark_type;
