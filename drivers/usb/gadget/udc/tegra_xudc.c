...
#define  MFINDEX_UFRAME_MASK 0x3
...
{
	dev_dbg(xudc->dev, "%s: lo = %#x, hi = %#x, status = %#x, control = %#x\n",
		 type, trb->data_lo, trb->data_hi, trb->status, trb->control);
}
...
static void tegra_xudc_device_mode_on(struct tegra_xudc *xudc)
{
	...
	tegra_xusb_utmi_set_vbus_override(xudc->utmi_phy);
	if (xudc->pullup) {
				   ...
				   struct usb_gadget_driver *driver)
{
	...
	if (xudc->pullup && xudc->device_mode) {
		val = xudc_readl(xudc, CTRL);
		val |= CTRL_ENABLE;
		xudc_writel(xudc, val, CTRL);
	}
...
{
	/* TODO: What to do with SEL values? */
}
...
{
	/* TODO: What to do with isoch delay? */
}
						  ...
						  struct tegra_xudc_trb *event)
{
	...
	if (req && (short_packet || !trb_read_chain(trb))) {
		...
		dev_dbg(xudc->dev, "bytes transfered %u / %u\n",
...
static int tegra_xudc_probe(struct platform_device *pdev)
{
	...
	xudc->ss_clk = devm_clk_get(&pdev->dev, "xusb_ss");
	...
	xudc->ss_rst = devm_reset_control_get(&pdev->dev, "xusb_ss");
	...
	xudc->utmi_phy = devm_phy_optional_get(&pdev->dev, "utmi");
...
static int tegra_xudc_unpowergate(struct tegra_xudc *xudc)
{
	...
	val |= BLCG_ALL;
	val &= ~(BLCG_DFPCI | BLCG_UFPCI | BLCG_FE | BLCG_COREPLL_PWRDN);
