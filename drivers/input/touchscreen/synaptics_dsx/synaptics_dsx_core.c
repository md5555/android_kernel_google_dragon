...
static struct device_attribute attrs[] = {
	__ATTR(full_pm_cycle, (0440 | 0220),
			synaptics_rmi4_full_pm_cycle_show,
			synaptics_rmi4_full_pm_cycle_store),
	__ATTR(reset, 0220,
			synaptics_rmi4_show_error,
			synaptics_rmi4_f01_reset_store),
	__ATTR(productinfo, 0440,
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	printk("Synaptics: suspend: %d enable_wakeup_gesture: %d", rmi4_data->suspend?1:0, rmi4_data->enable_wakeup_gesture?1:0);
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
		...
		printk("Synaptics: detected gestures: %d", retval);
...
/*
	printk("Synaptics: suspend: %d enable_wakeup_gesture: %d\n", scr_is_suspended?1:0, rmi4_data->enable_wakeup_gesture?1:0);

	if (scr_is_suspended && rmi4_data->enable_wakeup_gesture) {
		...
		printk("Synaptics: detected gestures: %d\n", retval);
		...
		if (detected_gestures) {
			...
			printk("Synaptics: gestures detected..!\n");
...
flash_prog_mode:
	...
	printk("Synaptics: Wake is enabled: %d", rmi4_data->enable_wakeup_gesture?1:0);
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	//wake_lock_init(&rmi4_data->wakelock, WAKE_LOCK_SUSPEND, "synapticsrmi4");
	...
	dev_pm_set_wake_irq(&(rmi4_data->pdev->dev), rmi4_data->irq); 
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	if (rmi4_data->enable_wakeup_gesture) {
		//wake_unlock(&rmi4_data->wakelock);
