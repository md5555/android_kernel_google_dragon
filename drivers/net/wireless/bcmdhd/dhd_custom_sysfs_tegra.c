...
tegra_sysfs_register(struct device *dev)
{
	...
	if (tegra_debugfs_root) {
		debugfs_create_file("scan", 0644,
			tegra_debugfs_root, (void *) 0,
			&tegra_debugfs_histogram_scan_fops);
		debugfs_create_file("tcpdump", 0644,
			tegra_debugfs_root, (void *) 0,
