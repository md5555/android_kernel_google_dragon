...
static void do_smc(struct te_request *request, struct tlk_device *dev)
{
	...
	pr_info("%s: type: 0x%08x, args: 0x%08x, params: 0x%08x\n", __func__,
		request->type, smc_args, smc_params);
	pr_info("request ptr: 0x%p sizeof is 0x%08lx\n", request,
		sizeof(struct te_request));
		    ...
		    struct tlk_context *context)
{
	...
	pr_info("request: 0x%p\n", request);
	pr_info("type        0x%08x\n", request->type);
	pr_info("session_id  0x%08x\n", request->session_id);
	pr_info("command_id  0x%08x\n", request->command_id);
	pr_info("params      0x%llx\n", request->params);
	pr_info("params_size 0x%08x\n", request->params_size);
	pr_info("dest_uuid   0x%08x\n", request->dest_uuid[0]);
