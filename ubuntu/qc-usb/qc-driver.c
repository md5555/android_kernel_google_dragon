...
module_param(subsample, int, 0444);
...
module_param(retryerrors, int, 0444);
...
static int qc_isoc_start(struct quickcam *qc)
{
	...
	for (b=0; b<ISOC_URBS; b++) {
		...
		urb->transfer_flags         = URB_ISO_ASAP;		/* Leave URB_ASYNC_UNLINK clear and never call usb_unlink_urb() */
...
fail5:	b = ISOC_URBS;
fail4:	while (--b >= 0) usb_kill_urb(id->urbs[b]);
...
static int qc_capt_get(struct quickcam *qc, unsigned char **frame)
{
	...
	unsigned char *rawdata=NULL;		/* Raw data from camera */
	int rawdatalen;
...
static ssize_t qc_v4l_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
	struct video_device *dev = video_devdata(file);
...
#endif
{
	static const Bool ignore_channel = TRUE;	/* RealProducer is reported to fail if driver doesn't accept channel change */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
	struct video_device *dev = video_devdata(file);
	void *argp = (void *)arg;
#endif
...
/* {{{ [fold] VIDIOCSCHAN:    Select channel to capture */
		case VIDIOCSCHAN:	/* Select channel to capture */
		{
			...
			if (!ignore_channel && i!=0) retval = -EINVAL;
			break;
		}
/* }}} */
