...
static void kevent(struct work_struct *ws)
{
	struct at76c503 *dev = container_of(ws, struct at76c503, kevent);
	int ret;
	unsigned long flags;
...
static void ieee80211_fixup(struct sk_buff *skb, int iw_mode)
{
	...
	i802_11_hdr = (struct ieee80211_hdr_3addr *)eth_hdr(skb);
	skb_pull(skb, sizeof(struct ieee80211_hdr_3addr));
//	skb_trim(skb, skb->len - 4); /* Trim CRC */
...
{	
	...
	dbg(DBG_RX_FRAGS_SKB, "%s: incoming skb: head %p data %p "
	    "tail %lu end %lu len %d",
	    dev->netdev->name, skb->head, skb->data, (unsigned long)skb->tail,
	    (unsigned long)skb->end, skb->len);
	...
	dbg(DBG_RX_FRAGS_SKB, "%s: trimmed skb: head %p data %p tail %lu "
	    "end %lu len %d data %p data_len %d",
	    dev->netdev->name, skb->head, skb->data, (unsigned long)skb->tail,
	    (unsigned long)skb->end, skb->len, data, data_len);
				 ...
				 char *extra)
{
	...
	list_for_each_safe(lptr, nptr, &(dev->bss_list)) {
		...
		for(i=0; i < curr_bss->rates_len; i++) {
			...
			/* Add new value to event */
			curr_val = iwe_stream_add_value(info, curr_pos, curr_val, 
...
static int at76c503_do_probe(struct module *mod, struct usb_device *udev,
			     const u8 *fw_data, int fw_size, u32 board_type,
			     const char *netdev_name)
{
