...
/* NOTE: This is NOT the module device table. We're just abusing the
 * pci_device_id struct for our own purposes */
static struct fw_match fw_ver_table[] = {
	{ /* Abocom WG24500, reported by Franz Pletz <info@franz-pletz.org> */
...
#define PCI_MATCH(a,b) (a == b || a == (unsigned short)PCI_ANY_ID)

/* Returns an alternate version of the firmware for specific devices. The
 * above table will _always_ be overriden by module param. */
static const char *get_firmware_ver(struct pci_dev *dev)
{
	struct fw_match *dev_match;

	if (!dev || strcmp(firmware_ver, "default"))
		goto use_default;

	for (dev_match = fw_ver_table; dev_match->vendor; dev_match++) {
		if (PCI_MATCH(dev_match->vendor, dev->vendor) &&
		    PCI_MATCH(dev_match->device, dev->device) &&
		    PCI_MATCH(dev_match->subvendor, dev->subsystem_vendor) &&
		    PCI_MATCH(dev_match->subdevice, dev->subsystem_device))
			return dev_match->verstr;
	}
...
acxpci_s_upload_fw(acx_device_t *adev)
{
	...
	printk(KERN_INFO "acx: loading firmware for acx1%02d chipset with radio"
		   " ID %02X\n", IS_ACX111(adev)*111, adev->radio_type);

	/* Try combined, then main image */
	adev->need_radio_fw = 0;
	snprintf(filename, sizeof(filename), "acx/%s/tiacx1%02dc%02X",
			 get_firmware_ver(adev->pdev),
...
acxpci_s_upload_radio(acx_device_t *adev)
{
	...
	printk(KERN_INFO "acx: loading radio image for radio %02X\n",
	       adev->radio_type);

	acx_s_interrogate(adev, &mm, ACX1xx_IE_MEMORY_MAP);
	offset = le32_to_cpu(mm.CodeEnd);

	snprintf(filename, sizeof(filename), "acx/%s/tiacx1%02dr%02X",
		 get_firmware_ver(adev->pdev), IS_ACX111(adev)*11,
		 adev->radio_type);
