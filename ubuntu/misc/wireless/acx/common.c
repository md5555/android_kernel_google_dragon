...
char firmware_ver[33] = "default";
module_param_string(firmware_ver, firmware_ver, sizeof(firmware_ver), 0444);
MODULE_PARM_DESC(firmware_ver, "Alternate firmware version.");
...
fw_stats_fail:
	st -= partlen;
	p += sprintf(p,
		"failed at %s part (size %u), offset %lu (struct size %u), "
		"please report\n", part_str, partlen,
		(unsigned long)st - (unsigned long)fw_stats, len);

fw_stats_bigger:
	for (; st < st_end; st += 4)
		p += sprintf(p,
			"UNKN%3lu: %u\n", (unsigned long)st - (unsigned long)fw_stats, *(u32 *)st);
...
#endif
	printk(KERN_INFO "acx: Loaded combined PCI/USB driver, firmware_ver=%s\n",
	       firmware_ver);
#if defined(CONFIG_NET_ACX_PCI)
	r1 = acxpci_e_init_module();
#else
	r1 = -EINVAL;
#endif
#if defined(CONFIG_NET_ACX_USB)
